...
<!--                <button class="layui-btn layuiadmin-btn-list" data-type="add">添加留言</button>-->
            ...
            <script type="text/html" id="buttonTpl">
                {{#  if(d.status == 1){ }}
                    <button class="layui-btn layui-btn-xs" onclick="note({{d.id}})">查看</button>
                {{#  } else { }}
                    <button class="layui-btn layui-btn-xs" onclick="note({{d.id}})">查看</button>
                {{#  } }}
            </script>
...
<script>
    ...
    function note(mId){
        layer.open({
            type: 2
            ,title: '查看信息'
            ,content: 'messageNote.php?mId='+mId
            ,maxmin: true
            ,area: ['100%','60%']
            ,btn: ['確定', '取消']
            ,yes: function(index, layero){
...
<script>
    layui.define(["table", "form"],
        function(t) {
            ...
            i.render({
                elem: "#orderList",
                url: "messageCon.php?status=<?php echo $status;?>",
                cols: [[{
                    type: "checkbox",
                    fixed: "left"
                },
