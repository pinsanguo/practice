...
if(!empty($_POST['username'])){
    ...
    $res = $mysql->field('id,name,password')
        ->where('name="'.$post['username'].'" and type = 2 ')
        ->select('user');
...
<!--                <label>免费注册</label>-->
                <label><a href="userAdd2.php" class="layadmin-user-jump-change layadmin-link">免费注册</a></label>
...
<script>
  layui.config({
    ...
    form.on('submit(LAY-user-login-submit)', function(obj){
        var field = obj.field;
        sendAjax(field,'userLogin2.php','/index.php');
    });
  });
</script>
<script>
